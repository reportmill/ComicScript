...
public class PlayerView extends ScaleBox {
    ...
    // Whether currently running all lines
    boolean _playing;
    ...
    public void scriptChanged()
    {
        ...
        if (runLine >= getScript().getLineCount()) runLine = getScript().getLineCount() - 1;
        _runLine = -1;
        setRunLine(runLine);
    ...
    int getLineCount()
    {
        return _script.getLineCount();
    }

    int getLineRunTime(int aLine)
    {
        return _script.getLineRunTime(aLine);
    }

    int getLineStartTime(int aLine)
    {
        return _script.getLineStartTime(aLine);
    }

    int getLineEndTime(int aLine)
    {
        return _script.getLineEndTime(aLine);
    }

    int getLineForTime(int aTime)
    {
        return _script.getLineForTime(aTime);
    }
    ...
    protected void playLineStart()
    {
        ...
        if (line != null && !line.isLoaded()) {
            line.addLoadListener(() -> ViewUtils.runLater(() -> playLine(getRunLine())));
            return;
        }
    ...
    {
        return _playBarShowing;
    }//_playBar!=null && _playBar.isShowing(); }
    ...
    protected void setShowingControls(boolean aValue)
    {
        ...
        if (aValue) {
            addChild(getPlayBar());
            getPlayBar().setOpacity(0);
            getPlayBar().getAnimCleared(300).setOpacity(1).play();
        } else {
    ...
    public void showIntroAnim()
    {
        ...
        _introView = new ColView();
        _introView.setAlign(Pos.TOP_CENTER);
        _introView.setSpacing(0);
