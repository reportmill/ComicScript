...
public class PlayBar extends RowView {
     ...
     * Sets the bar height.
     */
    void setBarHeight(double aValue)
    {
        _barHeight = aValue;
        repaint();
    }
    ...
    protected void paintFront(Painter aPntr)
    {
        ...
        aPntr.setColor(new Color(1, 1, 1, .4));
        aPntr.fillRect(x, barY, w, barH);
        ...
        aPntr.fillRect(x, barY, lineW, barH);

        // Draw progress bar frames
        aPntr.setColor(MARK_COLOR);
        for (int i = 0; i < lineCount - 1; i++) {
            int rt = _player.getLineEndTime(i);
        ...
        aPntr.setColor(Color.RED);
        aPntr.fill(new Ellipse(x + lineW - btnH / 2, BAR_MIDY - btnH / 2, btnH, btnH));
    }
    ...
    protected void playerDidFrame()
    {
        ...
        sb.append(prs);
        sb.append(" / ").append(pmm);
        if (pms < 10) sb.append("0");
        sb.append(pms);
        _timeLabel.setText(sb.toString());
        repaint();
    }
    ...
    public static class PlayButtonBig extends View {
        ...
        protected void paintFront(Painter aPntr)
        {
            ...
            aPntr.setColor(new Color(0, 0, 0, .1));
            aPntr.fill(c1);
            ...
            if (_playing) {
                ...
                path.lineBy(-pw / 4, 0);
                path.close();
                path.moveTo(w / 2 + pw * .5 / 4, h / 2 - ph / 2);
                path.lineBy(pw / 4, 0);
                path.lineBy(0, ph);
                path.lineBy(-pw / 4, 0);
                path.close();
