...
public class CameraView extends ScaleBox implements Star {
    ...
    protected void layoutImpl()
    {
        ...
        ParentView aPar = this;
        View aChild = getContent();
        if (aChild == null) return;
        Insets theIns = null;
        boolean isFillWidth = false;
        boolean isFillHeight = false;
        ...
        double pw = aPar.getWidth() - px - ins.right;
        if (pw < 0) pw = 0;
        if (pw <= 0) return;
        double ph = aPar.getHeight() - py - ins.bottom;
        if (ph < 0) ph = 0;
        if (ph <= 0) return;
        ...
        if (isFillWidth || isFillHeight || cw > pw || ch > ph) {
            ...
            aChild.setScaleX(sx * _zoom);
            aChild.setScaleY(sy * _zoom);
            return;
        }
        ...
        aChild.setScaleX(_zoom);
        aChild.setScaleY(_zoom);
    }
