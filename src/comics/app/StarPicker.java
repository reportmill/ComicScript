...
public class StarPicker extends ViewOwner {
    ...
    protected void initUI()
    {
        ...
        // Get StarsBrowser
        _starsBrowser = getView("StarsBrowser", BrowserView.class);
        _starsBrowser.setRowHeight(24);
    ...
    protected List<Star> getStarsForPlayer(PlayerView aPlayer)
    {
        ...
        stars.add(set);
        Star cam = aPlayer.getCamera();
        stars.add(cam);

        // Iterate over lines
        for (ScriptLine sline : script.getLines()) {
            Star star = sline.getStar();
    ...
    public class StarListView extends RowView {
        ...
        public void setSelIndex(int anIndex)
        {
            ...
            StarView sv = getStarView(_selIndex);
            if (sv != null) sv.setEffect(null);
            _selIndex = anIndex;
            StarView sv2 = getStarView(_selIndex);
            if (sv2 != null) sv2.setEffect(SELECT_EFFECT);
            repaint();
        }
         ...
         */
        public Star getSelStar()
        {
            return getStar(_selIndex);
        }

        /**
         * Selects the given Star.
         */
        public void setSelStar(Star aStar)
        {
            int ind = getStarIndex(aStar);
            setSelIndex(ind);
        }
         ...
         */
        protected Star getStar(int anIndex)
        {
            return anIndex >= 0 ? _stars.get(anIndex) : null;
        }

        /**
         * Returns the index of given Star.
         */
        protected int getStarIndex(Star aStar)
        {
            return _stars.indexOf(aStar);
        }
    ...
    private class StarView extends ImageView {
        ...
        protected void paintFront(Painter aPntr)
        {
            ...
            aPntr.setColor(isMouseOver() || getEffect() != null ? Color.BLUE : Color.GRAY);
            aPntr.draw(rrect);
            aPntr.clipRect(0, h - 12, w, 12);
            aPntr.fill(rrect);
            aPntr.setColor(Color.WHITE);
            String name = _star.getStarName();
            Font font = Font.Arial10;
            aPntr.setFont(font);
